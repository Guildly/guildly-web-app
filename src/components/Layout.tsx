...
export default function Layout({
  ...
  const { data } = useStarkName({
    address: pid?.toString().startsWith("0x") ? pid.toString() : "0x0",
  });
  ...
  const backTitle = titles[titles.length - 2];
  let backPageTitle = "";
  if (backTitle) {
    const backPageTitleWords = backTitle.split("-");
    for (let i = 0; i < backPageTitleWords.length; i++) {
      let titleWord = capitaliseFirst(backPageTitleWords[i]);
      backPageTitle += titleWord + " ";
    }
  }
          ...
          <div
            className={
              isLeftMenuOpen && !isRightMenuOpen
                ? [styles.main_header_area, styles.right_collapsed].join(" ")
                : !isLeftMenuOpen && isRightMenuOpen
                ? [styles.main_header_area, styles.left_collapsed].join(" ")
                : !isLeftMenuOpen && !isRightMenuOpen
                ? [styles.main_header_area, styles.both_collapsed].join(" ")
                : styles.main_header_area
            }
          >
